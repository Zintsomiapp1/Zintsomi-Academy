    ...
    return (
      <div className={`flex items-center ${isMobile ? 'space-y-2 flex-col' : 'space-x-3'}`}>
        ...
        <Button 
          ...
          size="sm" 
          onClick={onSignOut}
          className={`flex items-center space-x-1 border-white text-white bg-transparent hover:bg-white hover:text-sky-600 ${isMobile ? 'w-full' : ''}`}
        >
  ...
  return (
    <div className={`flex items-center ${isMobile ? 'space-y-2 flex-col' : 'space-x-3'}`}>
      ...
      <Link to="/auth">
        <Button 
          size="sm" 
          className={`bg-white text-sky-600 hover:bg-gray-100 ${isMobile ? 'w-full' : ''}`}
        >
          Get Started
        </Button>
      </Link>
